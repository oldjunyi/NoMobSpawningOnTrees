...
public class RuleLight extends Rule {
	...
	public boolean apply(SpawningEntry entry) {
		if (type == 0) {
			...
			int skyLight = entry.world.getSavedLightValue(EnumSkyBlock.Sky, entry.x, entry.y + 1, entry.z) - skyLightSub;
			int blockLight = entry.world.getSavedLightValue(EnumSkyBlock.Block, entry.x, entry.y + 1, entry.z);
